...
class TaxOptimizer:
    ...
    async def select_tax_lots(
        ...
        try:
            ...
            symbol_positions = [
                p for p in positions if p.symbol == symbol and p.tax_lot_id is not None
            ]
            ...
            if method == TaxMethod.FIFO:
                ...
                symbol_positions.sort(
                    key=lambda p: p.acquisition_date or p.created_at, reverse=True
                )
            elif method == TaxMethod.HIFO:
                # Highest In First Out - highest cost basis first (best for tax)
                symbol_positions.sort(
                    key=lambda p: p.cost_basis or p.entry_price, reverse=True
                )
                ...
                selected_lots.append(
                    {
                        ...
                        "acquisition_date": (
                            ...
                            else position.created_at.isoformat()
                        ),
                    }
                )
    ...
    async def optimize_year_end_positions(
        ...
        if current_net > target_net_gain:
            loss_positions = [p for p in positions if p.pnl_dollars < 0]
        ...
        # If we have losses, look for gains to realize
            ...
            gain_positions = [p for p in positions if p.pnl_dollars > 0]
