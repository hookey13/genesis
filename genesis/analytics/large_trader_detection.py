...
class LargeTraderDetector:
    ...
    async def _update_vpin(self, symbol: str, volume: Decimal, side: str) -> Decimal:
        ...
        if (
            not buckets
            or sum(b[0] + b[1] for b in [buckets[-1]]) >= self.vpin_bucket_size
        ):
            # Start new bucket
            if side == "buy":
    ...
    def _find_or_create_cluster(
        ...
        for cluster_id, cluster in self.active_clusters.items():
            ...
            if (
                cluster.end_time
                and current_time - cluster.end_time <= self.cluster_time_window
            ):
        ...
        cluster = TradeCluster(
            cluster_id=cluster_id, symbol=symbol, start_time=current_time
        )
    ...
    def _is_similar_pattern(
        self, activity: WhaleActivity, cluster: TradeCluster
    ) -> bool:
    ...
    def _cleanup_old_clusters(self, current_time: datetime) -> None:
        ...
        for cluster_id, cluster in self.active_clusters.items():
            if (
                cluster.end_time
                and current_time - cluster.end_time > self.cluster_time_window * 2
            ):
    ...
    def _update_entity_tracking(self, whale_activity: WhaleActivity) -> None:
        ...
        entity_id = (
            whale_activity.cluster_id or f"whale_{whale_activity.timestamp.timestamp()}"
        )
    ...
    def get_whale_statistics(self, symbol: str) -> Dict[str, any]:
        ...
        return {
            ...
            "active_clusters": len(
                [c for c in self.active_clusters.values() if c.symbol == symbol]
            ),
