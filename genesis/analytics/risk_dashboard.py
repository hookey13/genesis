...
class RiskDashboard:
    ...
    async def _calculate_parametric_var(
        ...
        weights = []
        for symbol in symbols:
            symbol_value = sum(p.dollar_value for p in positions if p.symbol == symbol)
    ...
    async def calculate_portfolio_greeks(self, account_id: str) -> dict[str, Decimal]:
        ...
        try:
            ...
            for position in positions:
                ...
                total_vega += abs(position.quantity) * Decimal(
                    "0.01"
                )  # Simplified vega
    ...
    async def update_risk_metrics(self, account_id: str) -> RiskMetrics:
        ...
        try:
            ...
            # Calculate returns
            returns = [
                float(t.pnl_percent) / 100 for t in trades if hasattr(t, "pnl_percent")
            ]
            ...
            if returns:
                ...
                sharpe = (
                    Decimal(str(mean_return / std_return))
                    if std_return > 0
                    else Decimal("0")
                )
                ...
                sortino = (
                    Decimal(str(mean_return / downside_dev))
                    if downside_dev > 0
                    else Decimal("0")
                )
