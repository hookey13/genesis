...
class OrderFlowAnalyzer:
    ...
    def _calculate_flow_velocity(self, symbol: str, current_flow: Decimal) -> Decimal:
        ...
        if time_diff.total_seconds() > 0:
            velocity = (current_flow - previous_metrics.net_flow) / Decimal(str(time_diff.total_seconds()))
        else:
            velocity = Decimal("0")
    ...
    def _calculate_pressure_score(
        ...
        if volume_ratio > 1:
            volume_score = min(Decimal("100"), Decimal(str(np.log(float(volume_ratio)))) * Decimal("30"))
        elif volume_ratio < 1 and volume_ratio > 0:
            volume_score = max(Decimal("-100"), Decimal(str(np.log(float(volume_ratio)))) * Decimal("30"))
        else:
            volume_score = Decimal("0")
        
        # Aggression score
        aggression_score = min(Decimal("100"), (aggression_ratio - Decimal("1")) * Decimal("20"))
        
        # Velocity score (normalized)
        velocity_normalized = max(Decimal("-100"), min(Decimal("100"), flow_velocity * Decimal("10")))
