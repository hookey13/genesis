...
class TwapAnalyzer:
    ...
    async def generate_execution_report(self, execution_id: str) -> TwapReport:
        ...
        try:
            ...
            report = TwapReport(
                ...
                completed_slices=len([s for s in slice_history if s['status'] == 'EXECUTED']),
                ...
                volume_weighted_participation=volume_metrics['volume_weighted_participation'],
                ...
                suggested_participation_rate=recommendations['suggested_participation_rate'],
    ...
    def _calculate_price_metrics(
        ...
        average_execution_price = total_value / total_quantity if total_quantity > 0 else Decimal("0")
        ...
        vwap_market_price = average_execution_price * Decimal("0.999")  # Slight improvement assumption
    ...
    def _calculate_performance_metrics(
        ...
        if arrival_price > 0:
            implementation_shortfall = ((avg_execution_price - arrival_price) / arrival_price) * Decimal("10000")  # in bps
        else:
            implementation_shortfall = Decimal("0")

        # TWAP effectiveness (how close to market TWAP)
        if vwap_market_price > 0:
            twap_effectiveness = (Decimal("1") - abs(twap_price - vwap_market_price) / vwap_market_price) * Decimal("100")
        else:
        ...
        avg_slippage_bps = total_slippage_bps / slice_count if slice_count > 0 else Decimal("0")

        # Total slippage cost
        executed_quantity = Decimal(str(execution_data['executed_quantity']))
        total_slippage_cost = (avg_slippage_bps / Decimal("10000")) * avg_execution_price * executed_quantity
    ...
    async def _estimate_market_impact(
        ...
        if total_volume > 0:
            ...
            estimated_impact_bps = impact_factor * volatility * impact_constant * Decimal("100")
    ...
    def _calculate_volume_metrics(self, slice_history: list[dict]) -> dict[str, Decimal]:
        ...
        volume_weighted_participation = weighted_participation / total_volume if total_volume > 0 else Decimal("0")
    ...
    def _calculate_timing_effectiveness(
        ...
        for slice_data in slice_history:
            if slice_data['status'] == 'EXECUTED':
                ...
                market_price = Decimal(str(slice_data.get('market_price', execution_price)))
                ...
                if side == OrderSide.BUY:
                    ...
                    if execution_price <= market_price:
                        score = min(Decimal("100"), (market_price - execution_price) / market_price * Decimal("1000"))
                    else:
                        score = max(Decimal("0"), Decimal("100") - (execution_price - market_price) / market_price * Decimal("1000"))
                else:
                    # Higher execution price is better for sells
                    if execution_price >= market_price:
                        score = min(Decimal("100"), (execution_price - market_price) / market_price * Decimal("1000"))
                    else:
                        score = max(Decimal("0"), Decimal("100") - (market_price - execution_price) / market_price * Decimal("1000"))

                timing_scores.append(score)

        overall_timing_score = sum(timing_scores) / len(timing_scores) if timing_scores else Decimal("50")
    ...
    def _calculate_risk_metrics(
        ...
        execution_risk_score = sum(risk_factors) / len(risk_factors) if risk_factors else Decimal("0")
        ...
        if quantities:
            ...
            concentration_risk = (sum(deviations) / len(deviations)) / avg_quantity * Decimal("100") if avg_quantity > 0 else Decimal("0")
        else:
    ...
    async def compare_executions(
        ...
        comparison = {
            ...
            'best_implementation_shortfall': min(r.implementation_shortfall for r in reports),
            'worst_implementation_shortfall': max(r.implementation_shortfall for r in reports),
            'avg_implementation_shortfall': sum(r.implementation_shortfall for r in reports) / len(reports),
