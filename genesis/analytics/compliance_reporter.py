...
class ComplianceReporter:
    ...
    async def extract_trade_audit_log(
        ...
        try:
            ...
            for event in events:
                if event.event_type in ["order.executed", "trade.completed", "position.opened", "position.closed"]:
                    entry = {
                        "audit_id": str(uuid4()),
                    ...
                    if event.event_type == "order.executed":
                        ...
                        entry.update({
                            ...
                            "quantity": event_data.get("quantity"),
                            "price": event_data.get("price"),
                            "pnl_dollars": event_data.get("pnl_dollars") if event.event_type == "position.closed" else None,
                        })
    ...
    async def _generate_mifid_ii_report(
        ...
        total_volume = sum(Decimal(str(t.quantity)) * Decimal(str(t.entry_price)) for t in trades)
        
        # Best execution analysis
        slippage_data = []
        for trade in trades:
            if hasattr(trade, "expected_price") and trade.expected_price:
                slippage = abs(Decimal(str(trade.entry_price)) - Decimal(str(trade.expected_price)))
        ...
        return {
            ...
            "transaction_reporting": {
                ...
                "average_trade_size": str(total_volume / total_trades) if total_trades > 0 else "0",
            },
            "best_execution": {
                "average_slippage": avg_slippage,
                "execution_quality_score": 100 - (avg_slippage * 100) if avg_slippage < 1 else 0,
            },
    ...
    async def _generate_emir_report(
        ...
        max_position_size = max((Decimal(str(p.dollar_value)) for p in positions), default=Decimal("0"))
    ...
    async def _generate_cftc_report(
        ...
        large_trades = [t for t in trades if Decimal(str(t.quantity)) * Decimal(str(t.entry_price)) > Decimal("50000")]
        ...
        return {
            ...
            "large_trader_reporting": {
                ...
                "total_reportable_volume": str(
                    sum(Decimal(str(t.quantity)) * Decimal(str(t.entry_price)) for t in large_trades)
                ),
            },
    ...
    async def _generate_best_execution_report(
        ...
        latencies = [o.latency_ms for o in orders if hasattr(o, "latency_ms") and o.latency_ms]
        ...
        for order in filled_orders:
            if order.side == OrderSide.BUY and hasattr(order, "expected_price"):
                improvement = Decimal(str(order.expected_price)) - Decimal(str(order.price))
            elif order.side == OrderSide.SELL and hasattr(order, "expected_price"):
                improvement = Decimal(str(order.price)) - Decimal(str(order.expected_price))
            else:
        ...
        avg_price_improvement = sum(price_improvements) / len(price_improvements) if price_improvements else 0
    ...
    async def _generate_transaction_cost_report(
        ...
        total_maker_fees = sum(
            Decimal(str(o.maker_fee_paid)) for o in orders 
            if hasattr(o, "maker_fee_paid") and o.maker_fee_paid
        )
        total_taker_fees = sum(
            Decimal(str(o.taker_fee_paid)) for o in orders 
        ...
        for order in orders:
            if hasattr(order, "slippage_percent") and order.slippage_percent:
                cost = Decimal(str(order.quantity)) * Decimal(str(order.price)) * Decimal(str(order.slippage_percent)) / 100
                slippage_costs += cost
        ...
        return {
            ...
            "total_transaction_costs": str(total_maker_fees + total_taker_fees + slippage_costs),
            "generated_at": datetime.now(timezone.utc).isoformat(),
        }
    ...
    async def validate_compliance_requirements(
        ...
        if "max_position_size" in compliance_settings:
            positions = await self.repository.get_positions_by_account(account_id)
            max_position = max(
                (Decimal(str(p.dollar_value)) for p in positions),
                default=Decimal("0")
            )
            results["position_limits"] = max_position <= Decimal(str(compliance_settings["max_position_size"]))
        ...
        if "max_daily_trades" in compliance_settings:
            ...
            results["daily_trade_limits"] = len(trades) <= compliance_settings["max_daily_trades"]
