...
class MarketManipulationDetector:
    ...
    async def track_order_cancellation(
        ...
        # Find original order
        if symbol not in self.active_orders or order_id not in self.active_orders[symbol]:
            return
    ...
    async def track_trade_execution(
        ...
        if order_id and symbol in self.active_orders and order_id in self.active_orders[symbol]:
            del self.active_orders[symbol][order_id]
    ...
    async def _detect_spoofing(
        ...
        if duration < timedelta(seconds=5):
            ...
            if cancellation_rate >= self.cancellation_threshold:
                ...
                avg_levels = sum(o.levels_from_best for o in recent_orders) / len(recent_orders)
                ...
                if confidence >= Decimal("0.6"):
                    pattern = ManipulationPattern(
                        ...
                        severity=self._determine_severity(confidence, original_order.notional)
                    )
        ...
        for side_orders in [bid_orders, ask_orders]:
            if len(side_orders) >= self.min_orders:
                ...
                if len(unique_prices) >= 3:  # Multiple price levels
                    ...
                    time_span = max(o.timestamp for o in side_orders) - min(o.timestamp for o in side_orders)
                        ...
                        if confidence >= Decimal("0.6"):
                            pattern = ManipulationPattern(
                                ...
                                severity=self._determine_severity(
                                    confidence,
                                    sum(o.notional for o in side_orders)
                                )
                            )
    ...
    def get_manipulation_statistics(self, symbol: str) -> Dict[str, any]:
        ...
        return {
            "cancellation_rate": float(self._calculate_cancellation_rate(recent_orders)),
