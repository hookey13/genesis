...
class HabitFuneralCeremony:
    ...
    async def get_suggested_habits(self, current_tier: str) -> list[str]:
    ...
    async def validate_ceremony_completion(
        ...
        # Check database
        record = (
            self.session.query(HabitFuneralRecord)
    ...
    async def get_ceremony_record(self, transition_id: str) -> CeremonyRecord | None:
        ...
        if record:
            ...
            old_habits = [OldHabit(**habit) for habit in json.loads(record.old_habits)]
            commitments = [
                CeremonyCommitment(
...
================================================================================
        ...
        record = (
            self.session.query(HabitFuneralRecord)
            .filter_by(transition_id=ceremony.transition_id)
            .first()
        )
    ...
    async def _store_ceremony(self, ceremony: CeremonyRecord) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to store ceremony", funeral_id=ceremony.funeral_id, error=str(e)
            )
            self.session.rollback()
            raise

    async def review_commitments(
        self, transition_id: str, days_since: int = 30
    ) -> dict[str, Any]:
