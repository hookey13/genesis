...
class VaultConfig(BaseSettings):
    ...
    vault_url: str = Field(
        default="http://localhost:8200", env="VAULT_URL", description="Vault server URL"
    )
    ...
    vault_token: str | None = Field(
        default=None, env="VAULT_TOKEN", description="Vault token for token auth"
    )
    vault_role_id: str | None = Field(
        default=None, env="VAULT_ROLE_ID", description="AppRole ID for approle auth"
    )
    vault_secret_id: str | None = Field(
        default=None, env="VAULT_SECRET_ID", description="AppRole secret ID"
    )
    ...
    verify_ssl: bool = Field(
        default=True, env="VAULT_VERIFY_SSL", description="Verify SSL certificates"
    )
    ca_cert_path: str | None = Field(
        default=None, env="VAULT_CA_CERT", description="Path to CA certificate bundle"
    )
