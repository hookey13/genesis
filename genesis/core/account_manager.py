...
class AccountManager:
    ...
    async def create_account(
        ...
        if account_type == AccountType.SUB:
            ...
            if parent.account_type != AccountType.MASTER:
                raise ValueError("Sub-accounts can only be created under MASTER accounts")
        ...
        try:
            ...
            # Publish event
            await self.event_bus.publish("account.created", {
    ...
    async def switch_account(self, account_id: str) -> bool:
        ...
        if not account:
            logger.error("account_switch_failed", account_id=account_id, reason="not_found")
            return False
        ...
        await self.event_bus.publish("account.switched", {
    ...
    async def update_account_balance(
        ...
        account = await self.get_account(account_id)
        if not account:
            logger.error("balance_update_failed", account_id=account_id, reason="not_found")
            return False
        ...
        await self.event_bus.publish("account.balance_updated", {
    ...
    async def check_account_permissions(
        self, account_id: str, permission: str
    ) -> bool:
        """
    ...
    async def update_account_permissions(
        ...
        account = await self.get_account(account_id)
        if not account:
            logger.error("permission_update_failed", account_id=account_id, reason="not_found")
            return False
        ...
        await self.event_bus.publish("account.permissions_updated", {
            "account_id": account_id,
    ...
    @requires_tier(TradingTier.STRATEGIST)
    async def aggregate_sub_account_balances(
        self, master_account_id: str
    ) -> Decimal:
        """
