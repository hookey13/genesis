...
class DisasterRecoveryValidator:
    ...
    async def _test_backup_procedures(self) -> dict[str, Any]:
        ...
        try:
            ...
            # Try to use restic if available
            restic_available = await self._check_restic_available()

            if restic_available:
                # Use restic for backup
                try:
                    # Initialize restic repository if needed
                    import os
                    repo_path = os.getenv("RESTIC_REPOSITORY", str(self.backup_dir / "restic-repo"))

                    # Check if repository exists
                    check_cmd = ["restic", "-r", repo_path, "snapshots", "--json"]
                    check_result = subprocess.run(check_cmd, capture_output=True, timeout=10)

                    if check_result.returncode != 0:
                        # Initialize repository
                        init_cmd = ["restic", "-r", repo_path, "init"]
                        subprocess.run(init_cmd, capture_output=True, timeout=10,
                                     env={**os.environ, "RESTIC_PASSWORD": "test_password"})

                    # Create backup with restic
                    backup_cmd = [
                        "restic", "-r", repo_path, "backup",
                        str(self.db_path.parent) if self.db_path.exists() else ".",
                        "--tag", "dr_test",
                        "--json"
                    ]
                    ...
                    backup_created = result.returncode == 0

                    # Also create tar backup for compatibility
                    ...
                    logger.warning(f"Restic backup failed, falling back to tar: {e}")
                    backup_created = False
            else:
                backup_created = False

            # Fallback to tar backup if restic not available or failed
    ...
    async def _check_restic_available(self) -> bool:
