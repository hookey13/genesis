...
class TestTierTransitionWorkflow:
    ...
    async def test_complete_tier_transition_sniper_to_hunter(
        self, transition_components, mock_session
    ):
        ...
        def setup_assessment_mocks(query_obj):
            if query_obj.__name__ == "TiltProfile":
                return MagicMock(
                    filter_by=lambda **k: MagicMock(first=lambda: mock_profile)
                )
            elif query_obj.__name__ == "Account":
                return MagicMock(
                    filter_by=lambda **k: MagicMock(first=lambda: mock_account)
                )
            elif query_obj.__name__ == "TiltEvent":
                return MagicMock(
                    filter=lambda *a: MagicMock(all=lambda: [], count=lambda: 1)
                )
                ...
                return MagicMock(
                    filter=lambda *a: MagicMock(
                        all=lambda: mock_trades, count=lambda: 100
                    )
                )
            elif query_obj.__name__ == "TierTransition":
                return MagicMock(
                    filter_by=lambda **k: MagicMock(first=lambda: mock_transition)
                )
            return MagicMock()
        ...
        report = await transition_components["assessor"].assess_readiness(
            profile_id=profile_id, target_tier="HUNTER"
        )
        ...
        paper_session_id = await transition_components[
            "paper_trading"
        ...
        for item in checklist_items:
            if item.is_required:
                response = (
                    f"Test response for {item.name} - " + "x" * item.min_response_length
                )
                await transition_components["checklist"].complete_item(
                    transition_id=transition_id, item_name=item.name, response=response
                )
        ...
        migration_plan = await transition_components[
            "strategy_loader"
        ].migrate_strategies(
            account_id=account_id, old_tier="SNIPER", new_tier="HUNTER"
        )
        ...
        adjustment_period_id = await transition_components[
            "adjustment_manager"
        ].start_adjustment_period(
            account_id=account_id,
            tier="HUNTER",
            duration_hours=48,
            transition_id=transition_id,
        ...
        status = await transition_components[
            "adjustment_manager"
        ].get_adjustment_status(account_id=account_id)
    ...
    async def test_transition_with_failures_and_retry(
        ...
        def setup_failing_mocks(query_obj):
            if query_obj.__name__ == "TiltProfile":
                ...
                return MagicMock(
                    filter=lambda *a: MagicMock(
                        all=lambda: mock_events, count=lambda: 5
                    )
                )
            elif query_obj.__name__ == "Trade":
                ...
                return MagicMock(
                    filter=lambda *a: MagicMock(
                        all=lambda: mock_trades, count=lambda: 10
                    )
                )
            return MagicMock()
    ...
    async def test_emergency_demotion_cancels_transition(
        ...
        success = await transition_components["adjustment_manager"].force_complete(
            account_id=account_id, reason="Emergency demotion"
        )
