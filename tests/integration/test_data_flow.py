...
from genesis.core.models import (
    Position, Order, Trade, MarketData, TierType, 
    OrderStatus, OrderType, OrderSide, Signal
)
...
class TestDataFlow:
    ...
    def mock_repository(self):
        ...
        repo.save_position = Mock(side_effect=lambda p: repo.positions.update({p.id: p}))
    ...
    @pytest.mark.asyncio
    async def test_market_data_ingestion_to_signal(self, mock_market_data, mock_repository):
        """Test market data flows to strategy signal generation."""
        # Setup strategy that generates signals
        strategy = Mock()
        strategy.name = "test_strategy"
        strategy.analyze = Mock(return_value=Signal(
