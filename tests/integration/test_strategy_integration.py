...
from genesis.core.models import (
    Position, Order, Signal, TierType, OrderStatus, 
    OrderType, OrderSide, ExecutionType
)
...
class TestStrategyIntegration:
    ...
    async def test_multi_strategy_concurrent_execution(self, orchestrator):
        ...
        strategies = [
            MockStrategy(f"strategy_{i}", f"BTC{i}USDT")
            for i in range(5)
        ]
    ...
    @pytest.mark.asyncio
    async def test_position_limits_across_strategies(self, orchestrator, mock_repository):
    ...
    async def test_ab_test_strategy_comparison(self, mock_repository):
        ...
        for i in range(10):
            # Strategy A results
            ab_test.record_result("momentum", {
            ...
            # Strategy B results
            ab_test.record_result("mean_reversion", {
                "pnl": Decimal(str(80 + i * 15)),
                "trade_count": 1,
                "win": i % 3 == 0
            })
