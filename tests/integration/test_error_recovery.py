...
from genesis.core.models import (
    Position, Order, TierType, OrderStatus, 
    OrderType, OrderSide, TradingState
)
...
class TestErrorRecovery:
    ...
    def mock_repository(self):
        ...
        repo.load_state = Mock(return_value={
            "positions": {},
            "orders": {},
    ...
    async def test_strategy_crash_isolation(self, mock_repository, mock_exchange):
        ...
        crashing_strategy.analyze = AsyncMock(
            side_effect=Exception("Strategy crashed")
        )
    ...
    async def test_exchange_error_handling(self, mock_exchange):
        ...
        for error_code, error_msg in error_scenarios:
            mock_exchange.place_order.side_effect = Exception(f"{error_code}: {error_msg}")
    ...
    async def test_network_partition_simulation(self, websocket_manager):
        ...
        with patch('websockets.connect', side_effect=ConnectionError("Network unreachable")):
