...
class TestMultiAccountInstitutionalWorkflows:
    ...
    async def setup_infrastructure(self):
        ...
        fix_gateway = FIXGateway(
            config={"sender_comp_id": "GENESIS", "target_comp_id": "BROKER"}
        )
        prime_broker = PrimeBrokerAdapter(config={"broker_type": "GOLDMAN_SACHS"})
        disaster_recovery = DisasterRecovery(
            repository=repo, exchange=exchange, event_bus=event_bus
        )
    ...
    @pytest.mark.asyncio
    async def test_multi_account_creation_and_hierarchy(
        self, setup_infrastructure, master_account, sub_accounts
    ):
        ...
        for sub in sub_accounts:
            created_sub = await account_manager.create_sub_account(
                parent_id=master.account_id, account_data=sub
            )
    ...
    @pytest.mark.asyncio
    async def test_multi_account_position_isolation(
        self, setup_infrastructure, sub_accounts
    ):
    ...
    @pytest.mark.asyncio
    async def test_risk_metrics_aggregation(
        self, setup_infrastructure, master_account, sub_accounts
    ):
    ...
    @pytest.mark.asyncio
    async def test_month_end_reconciliation_workflow(
        self, setup_infrastructure, sub_accounts
    ):
        ...
        repo.get_account_balance = AsyncMock(
            side_effect=lambda acc_id: db_balances.get(acc_id)
        )
        exchange.fetch_balance = AsyncMock(
            side_effect=lambda acc_id: exchange_balances.get(acc_id)
        )
    ...
    async def test_tax_optimization_workflow(self, setup_infrastructure):
        ...
        assert (
            optimal_lots[0]["lot_id"] == "LOT_002"
        )  # Higher cost basis selected first

        # Calculate tax impact
        tax_impact = await tax_optimizer.calculate_tax_impact(
            lots_to_sell=optimal_lots, sale_price=Decimal("45000.00")
        )
    ...
    async def test_disaster_recovery_workflow(self, setup_infrastructure, sub_accounts):
        ...
        with patch.object(disaster_recovery, "detect_failure", return_value=True):
            ...
            open_positions = [
                Position(
                    position_id="POS_001", account_id="SUB_001", size=Decimal("1.0")
                ),
                Position(
                    position_id="POS_002", account_id="SUB_002", size=Decimal("10.0")
                ),
            ]
    ...
    @pytest.mark.asyncio
    async def test_account_switching_audit_trail(
        self, setup_infrastructure, master_account, sub_accounts
    ):
        ...
        await account_manager.switch_account(
            from_account="MASTER_001", to_account="SUB_001", user_id="USER_001"
        )

        await account_manager.switch_account(
            from_account="SUB_001", to_account="SUB_002", user_id="USER_001"
        )
    ...
    @pytest.mark.asyncio
    async def test_institutional_feature_access_control(
        self, setup_infrastructure, master_account
    ):
        ...
        hunter_account = Account(
            account_id="HUNTER_001", tier=Tier.HUNTER, balance_usdt=Decimal("5000.00")
        )

        assert not await infra["account_manager"].can_use_multi_account(hunter_account)
        assert not await infra["compliance_reporter"].can_generate_reports(
            hunter_account
        )
        assert not await infra["risk_dashboard"].can_view_advanced_metrics(
            hunter_account
        )
    ...
    async def test_decimal_precision_across_workflow(self, setup_infrastructure):
        ...
        risk_result = await infra["risk_dashboard"].calculate_position_risk(
            position_value=precise_value, volatility=Decimal("0.02")
        )
        assert isinstance(risk_result, Decimal)

        # Tax calculation
        tax_result = await infra["tax_optimizer"].calculate_tax_on_gain(
            gain_amount=precise_value, tax_rate=Decimal("0.23")
        )
