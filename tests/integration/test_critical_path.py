...
class TestCriticalPath:
    ...
    async def test_complete_order_lifecycle(
        self, orchestrator, mock_exchange, mock_repository
    ):
        ...
        mock_exchange.place_order.return_value = {"orderId": order_id, "status": "NEW"}
        ...
        position.realized_pnl = (
            Decimal(close_result["avgPrice"]) - position.entry_price
        ) * position.quantity
    ...
    @pytest.mark.asyncio
    async def test_risk_limits_prevent_overleveragee(
        self, risk_engine, mock_repository
    ):
    ...
    async def test_performance_attribution_accuracy(self, mock_repository):
        ...
        attribution = await perf_engine.calculate_attribution(
            start_date=datetime.utcnow() - timedelta(days=2), end_date=datetime.utcnow()
        )

        # Calculate expected values
        btc_pnl = (Decimal("51000") - Decimal("50000")) * Decimal("0.1") - Decimal(
            "10.1"
        )
