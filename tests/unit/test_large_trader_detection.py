...
class TestTradeCluster:
    ...
    def test_cluster_creation(self):
        ...
        cluster = TradeCluster(cluster_id="BTCUSDT_123", symbol="BTCUSDT")
...
class TestLargeTraderDetector:
    ...
    def test_calculate_size_percentile(self, detector):
        ...
        # Test various sizes
        assert detector._calculate_size_percentile("BTCUSDT", Decimal("50")) == Decimal(
            "49"
        )
        assert detector._calculate_size_percentile("BTCUSDT", Decimal("95")) == Decimal(
            "94"
        )
        assert detector._calculate_size_percentile(
            "BTCUSDT", Decimal("150")
        ) == Decimal("100")
    ...
    def test_confidence_calculation(self, detector):
        ...
        confidence = detector._calculate_confidence(
            percentile=Decimal("99"), vpin=Decimal("0.5")
        )
        assert confidence > Decimal("0.8")

        # Medium confidence
        confidence = detector._calculate_confidence(
            percentile=Decimal("95"), vpin=Decimal("0.3")
        )
        assert Decimal("0.5") < confidence < Decimal("0.8")

        # Low confidence
        confidence = detector._calculate_confidence(
            percentile=Decimal("91"), vpin=Decimal("0.1")
        )
