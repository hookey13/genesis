...
class TestFlowWindow:
    ...
    def test_volume_calculations(self):
        ...
        # Add various trades
        trades = [
            TradeFlow(datetime.now(timezone.utc), Decimal("50000"), Decimal("1"), "buy", True),
            TradeFlow(datetime.now(timezone.utc), Decimal("50001"), Decimal("2"), "buy", False),
            TradeFlow(datetime.now(timezone.utc), Decimal("49999"), Decimal("1.5"), "sell", True),
            TradeFlow(datetime.now(timezone.utc), Decimal("49998"), Decimal("0.5"), "sell", False),
        ]
...
class TestOrderFlowAnalyzer:
    ...
    def test_calculate_metrics(self, analyzer):
        ...
        trades = [
            TradeFlow(datetime.now(timezone.utc), Decimal("50000"), Decimal("1"), "buy", True),
            TradeFlow(datetime.now(timezone.utc), Decimal("49999"), Decimal("1"), "sell", True),
        ]
    ...
    def test_confidence_calculation(self, analyzer):
        ...
        for _ in range(150):
            window.trades.append(TradeFlow(
    ...
    def test_cumulative_flow(self, analyzer):
        ...
        window = FlowWindow(window_size=timedelta(minutes=5))
        trades = [
            TradeFlow(datetime.now(timezone.utc), Decimal("50000"), Decimal("3"), "buy", True),
            TradeFlow(datetime.now(timezone.utc), Decimal("49999"), Decimal("1"), "sell", True),
        ]
        for trade in trades:
            window.add_trade(trade)
