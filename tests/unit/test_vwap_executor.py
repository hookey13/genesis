...
from genesis.engine.executor.vwap import ExecutionMode, VWAPExecutor, VWAPSlice
...
class TestVWAPExecutor:
    ...
    @pytest.mark.asyncio
    async def test_execute_vwap_tier_restriction(
        self,
        mock_exchange_gateway,
        mock_volume_analyzer,
        mock_vwap_tracker,
    ...
    async def test_execute_vwap_success(
        self,
        vwap_executor,
        mock_volume_analyzer,
        mock_vwap_tracker,
        ...
        # Setup mocks
        mock_volume_analyzer.predict_intraday_volume.return_value = (
            sample_volume_prediction
        )
    ...
    @pytest.mark.asyncio
    @patch.dict("os.environ", {"PYTEST_CURRENT_TEST": "test"})
    async def test_calculate_slices(
        self, vwap_executor, sample_order, sample_volume_prediction
    ):
        """Test slice calculation logic."""
        vwap_executor.volume_analyzer.get_optimal_participation_rate = Mock(
            return_value={
                0: Decimal("0.1"),
                30: Decimal("0.1"),
                60: Decimal("0.1"),
        ...
        for slice_obj in slices:
            assert slice_obj.quantity >= vwap_executor.min_slice_size_usd / Decimal(
                "100"
            )

    @pytest.mark.asyncio
    @patch.dict("os.environ", {"PYTEST_CURRENT_TEST": "test"})
    async def test_calculate_slices_aggressive_mode(
        self, vwap_executor, sample_order, sample_volume_prediction
    ):
        """Test slice calculation in aggressive mode."""
        vwap_executor.volume_analyzer.get_optimal_participation_rate = Mock(
            return_value={0: Decimal("0.1"), 30: Decimal("0.1")}
        )
    ...
    @pytest.mark.asyncio
    @patch.dict("os.environ", {"PYTEST_CURRENT_TEST": "test"})
    async def test_calculate_slices_passive_mode(
        self, vwap_executor, sample_order, sample_volume_prediction
    ):
        ...
        if len(slices_passive) > 0 and len(slices_normal) > 0:
            ...
            assert slices_passive[0].quantity <= slices_normal[0].quantity * Decimal(
                "1.2"
            )
    ...
    async def test_execute_single_slice_failure_retry(self, vwap_executor):
        ...
        exec_results = [
            ExecutionResult(
                success=False, order=Mock(), message="Failed", error="Network error"
            ),
            ExecutionResult(
                success=True,
                order=Mock(filled_quantity=Decimal("100")),
        ...
        with patch.object(
            vwap_executor, "execute_market_order", side_effect=exec_results
        ):
            await vwap_executor._execute_single_slice(
                slice_obj, parent_order, ExecutionMode.NORMAL, False
            )
    ...
    def test_get_order_type(self, vwap_executor):
        ...
        assert (
            vwap_executor._get_order_type(ExecutionMode.PASSIVE)
            == OrderType.LIMIT_MAKER
        )
        assert (
            vwap_executor._get_order_type(ExecutionMode.AGGRESSIVE) == OrderType.MARKET
        )
    ...
    def test_build_final_result(self, vwap_executor, sample_order):
        ...
        result = vwap_executor._build_final_result(
            sample_order, slices, "Test complete"
        )
    ...
    async def test_cancel_all_orders(self, vwap_executor):
        ...
        vwap_executor._active_executions["order_2"] = [Mock(symbol=Symbol("ETH/USDT"))]
