...
def sample_klines_data():
    ...
    for day in range(30):
        for hour in range(0, 24, 1):  # Hourly data
            ...
            volume = (
                1000 + (hour * 100) + np.random.randint(-200, 200)
            )  # Volume pattern
            data.append(
                [
...
class TestVolumeAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_fetch_historical_volume(
        self, volume_analyzer, mock_exchange_gateway, sample_klines_data
    ):
        """Test fetching historical volume data."""
        mock_exchange_gateway.get_klines.return_value = sample_klines_data

        symbol = Symbol("BTC/USDT")
        df = await volume_analyzer.fetch_historical_volume(
            symbol, days=30, interval="30m"
        )
    ...
    @pytest.mark.asyncio
    async def test_calculate_typical_profile(
        self, volume_analyzer, mock_exchange_gateway
    ):
        ...
        mock_df = pd.DataFrame(
            {
                "timestamp": pd.date_range(
                    start="2024-01-01", periods=48, freq="30min", tz="UTC"
                ),
        ...
        with patch.object(
            volume_analyzer, "fetch_historical_volume", return_value=mock_df
        ):
            symbol = Symbol("BTC/USDT")
            profile = await volume_analyzer.calculate_typical_profile(
    ...
    @pytest.mark.asyncio
    async def test_calculate_typical_profile_caching(
        self, volume_analyzer, mock_exchange_gateway
    ):
        ...
        with patch.object(
            ...
            profile1 = await volume_analyzer.calculate_typical_profile(
                symbol, lookback_days=30
            )
            assert mock_fetch.call_count == 1

            # Second call should use cache
            profile2 = await volume_analyzer.calculate_typical_profile(
                symbol, lookback_days=30
            )
    ...
    async def test_predict_intraday_volume(self, volume_analyzer):
        ...
        with patch.object(
            volume_analyzer, "calculate_typical_profile", return_value=mock_profile
        ):
            with patch.object(
                volume_analyzer, "fetch_historical_volume", return_value=mock_recent_df
            ):
                symbol = Symbol("BTC/USDT")
                current_time = datetime.now(UTC).replace(hour=12, minute=0)
                ...
                assert len(prediction.confidence_scores) == len(
                    prediction.predicted_buckets
                )
    ...
    async def test_analyze_volume_spike(self, volume_analyzer):
        ...
        mock_profile = VolumeProfile(
            ...
            date=datetime.now(UTC),
            total_volume=Decimal("48000"),
            time_buckets={0: Decimal("1000"), 30: Decimal("1000"), 60: Decimal("1000")},
            cumulative_percentages={},
        )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_fetch_historical(
        self, volume_analyzer, mock_exchange_gateway
    ):
