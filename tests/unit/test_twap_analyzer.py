...
class TestTwapAnalyzer:
    ...
    def test_calculate_price_metrics(
        self, analyzer, sample_execution_data, sample_slice_history
    ):
        """Test price metrics calculation."""
        metrics = analyzer._calculate_price_metrics(
            sample_execution_data, sample_slice_history
        )
    ...
    def test_calculate_performance_metrics(
        ...
        metrics = analyzer._calculate_performance_metrics(
            sample_execution_data, sample_slice_history, price_metrics
        )
    ...
    @pytest.mark.asyncio
    async def test_estimate_market_impact(
        self, analyzer, sample_execution_data, sample_slice_history
    ):
        """Test market impact estimation."""
        impact = await analyzer._estimate_market_impact(
            sample_execution_data, sample_slice_history
        )
        ...
        assert (
            impact["temporary_impact_bps"] + impact["permanent_impact_bps"]
            == impact["estimated_impact_bps"]
        )
    ...
    def test_calculate_volume_metrics(self, analyzer, sample_slice_history):
        ...
        assert (
            metrics["min_participation_rate"]
            <= metrics["avg_participation_rate"]
            <= metrics["max_participation_rate"]
        )

    def test_calculate_timing_effectiveness(
        self, analyzer, sample_execution_data, sample_slice_history
    ):
    ...
    def test_generate_recommendations(self, analyzer, sample_execution_data):
        ...
        recommendations = analyzer._generate_recommendations(
            sample_execution_data, performance_metrics, volume_metrics, risk_metrics
        )
    ...
    def test_generate_recommendations_high_slippage(
        self, analyzer, sample_execution_data
    ):
        ...
        assert (
            ...
            > sample_execution_data["duration_minutes"]
        )
        assert (
            recommendations["optimal_slice_count"]
            > sample_execution_data["slice_count"]
        )
        assert (
            recommendations["suggested_participation_rate"]
            < volume_metrics["avg_participation_rate"]
        )
        assert len(recommendations["improvement_opportunities"]) > 3  # Multiple issues

    @pytest.mark.asyncio
    async def test_generate_execution_report(
        self, analyzer, mock_repository, sample_execution_data, sample_slice_history
    ):
        """Test full report generation."""
        mock_repository.get_twap_execution = AsyncMock(
            return_value=sample_execution_data
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_execution_report_no_slices(
        self, analyzer, mock_repository, sample_execution_data
    ):
    ...
    async def test_compare_executions(self, analyzer, mock_repository):
        ...
        mock_repository.get_twap_execution = AsyncMock(
            side_effect=[execution1, execution2]
        )
    ...
    @pytest.mark.asyncio
    async def test_compare_executions_insufficient_data(
        self, analyzer, mock_repository
    ):
