...
class TestMigration008:
    ...
    def test_twap_executions_columns(self, db_session):
        ...
        expected_columns = {
            ...
            'average_price', 'twap_price', 'implementation_shortfall', 'participation_rate',
            'status', 'early_completion', 'early_completion_reason', 'started_at',
            'completed_at', 'paused_at', 'resumed_at', 'created_at', 'updated_at'
        }
    ...
    def test_twap_slice_history_columns(self, db_session):
        ...
        expected_columns = {
            ...
            'executed_quantity', 'execution_price', 'market_price', 'slippage_bps',
            'volume_at_execution', 'participation_rate', 'market_impact_bps',
            'time_delay_ms', 'order_id', 'client_order_id', 'status',
    ...
    def test_can_insert_twap_execution(self, db_session):
        ...
        result = db_session.execute(text(
            "SELECT * FROM twap_executions WHERE execution_id = :id"
        ), {'id': execution_id})
    ...
    def test_can_insert_twap_slice(self, db_session):
        ...
        result = db_session.execute(text(
            "SELECT * FROM twap_slice_history WHERE slice_id = :id"
        ), {'id': slice_id})
