...
def test_dockerfile_configuration():
    ...
    assert "FROM python:3.11.8" in dockerfile_content, "Dockerfile not using Python 3.11.8"
    ...
    assert "requirements.txt" in dockerfile_content, "Dockerfile not installing requirements"
...
def test_docker_compose_files():
    ...
    assert "postgres:" in prod_content, "docker-compose.prod.yml missing postgres service"
    assert "restart: always" in prod_content, "Production compose missing restart policy"
...
def test_requirements_files_structure():
    ...
    assert "-r base.txt" in sniper_req.read_text(), "sniper.txt not referencing base.txt"
    
    hunter_req = project_root / "requirements" / "hunter.txt"
    assert hunter_req.exists(), "requirements/hunter.txt does not exist"
    assert "-r base.txt" in hunter_req.read_text(), "hunter.txt not referencing base.txt"
...
def test_pyproject_toml_configuration():
    ...
    assert "[tool.ruff]" in pyproject_content, "pyproject.toml missing ruff configuration"
    assert "[tool.black]" in pyproject_content, "pyproject.toml missing black configuration"
    assert "[tool.mypy]" in pyproject_content, "pyproject.toml missing mypy configuration"
    assert "[tool.pytest.ini_options]" in pyproject_content, "pyproject.toml missing pytest configuration"
    assert "[tool.coverage.run]" in pyproject_content, "pyproject.toml missing coverage configuration"
    
    # Check specific settings
    assert 'line-length = 88' in pyproject_content, "Incorrect line length configuration"
    assert 'target-version = "py311"' in pyproject_content, "Incorrect Python version target"
    assert 'python_version = "3.11"' in pyproject_content, "Incorrect mypy Python version"