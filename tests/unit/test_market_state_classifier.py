...
from genesis.analytics.volatility_calculator import (
...
def sample_candles():
    ...
    for i in range(30):
        ...
        candle = Candle(
            open_time=datetime.now(timezone.utc) - timedelta(hours=30-i),
            open=open_price,
            high=high,
            low=low,
            close=close,
            volume=Decimal("1000") + Decimal(str(i * 10)),
            close_time=datetime.now(timezone.utc) - timedelta(hours=29-i),
...
class TestMarketStateClassifier:
    ...
    def test_detect_volume_pattern_anomaly(self, classifier):
        ...
        for i in range(24):
            timestamp = datetime.now(timezone.utc).replace(hour=i)
            classifier.update_volume_profile(symbol, Decimal("1000"), timestamp)
        
        # Test normal volume
        is_anomaly, reason = classifier.detect_volume_pattern_anomaly(
            symbol, Decimal("1100"), datetime.now(timezone.utc)
        )
        assert not is_anomaly
        
        # Test anomalous volume
        is_anomaly, reason = classifier.detect_volume_pattern_anomaly(
            symbol, Decimal("10000"), datetime.now(timezone.utc)
        )
        assert is_anomaly
