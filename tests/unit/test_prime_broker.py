...
class TestPrimeBrokerAdapter:
    ...
    async def test_multi_venue_order_routing(self, gs_adapter, sample_order):
        ...
        venue_allocations = [
            VenueAllocation(
                venue="BINANCE", quantity=Decimal("5.0"), price=Decimal("44999.00")
            ),
            VenueAllocation(
                venue="COINBASE", quantity=Decimal("3.0"), price=Decimal("45001.00")
            ),
            VenueAllocation(
                venue="KRAKEN", quantity=Decimal("2.0"), price=Decimal("45000.50")
            ),
        ]

        with patch.object(
            gs_adapter, "calculate_smart_routing", return_value=venue_allocations
        ):
    ...
    async def test_best_execution_analysis(self, ms_adapter):
        ...
        market_data = {
            "CME": {
                "bid": Decimal("44998.00"),
                "ask": Decimal("45002.00"),
        ...
        with patch.object(ms_adapter, "fetch_market_data", return_value=market_data):
            ...
            best_venue = await ms_adapter.find_best_execution(
                symbol="BTC/USD", side=OrderSide.BUY, quantity=Decimal("10.0")
            )
    ...
    async def test_position_aggregation_across_venues(self, gs_adapter):
        ...
        with patch.object(
            gs_adapter,
            "fetch_venue_positions",
            side_effect=lambda v: venue_positions[v],
        ):
            # Execute
            ...
            expected_avg = (
                Decimal("5.0") * Decimal("45000.00")
                + Decimal("3.0") * Decimal("44950.00")
                + Decimal("2.0") * Decimal("45100.00")
            ) / Decimal("10.0")
    ...
    async def test_risk_limit_checking(self, gs_adapter):
        ...
        with patch.object(gs_adapter, "get_risk_limits", return_value=risk_limits):
            ...
            order2 = Order(
                symbol="BTC/USD", side=OrderSide.BUY, quantity=Decimal("20.0")
            )
    ...
    async def test_margin_requirement_calculation(self, gs_adapter):
        ...
        assert margin.maintenance_margin == expected_margin * Decimal(
            "0.5"
        )  # Typical 50% maintenance
    ...
    async def test_connection_failover(self, gs_adapter):
        ...
        with patch("aiohttp.ClientSession.post") as mock_post:
            ...
            connection = await gs_adapter.connect_with_failover(
                primary=primary_endpoint, backups=backup_endpoints
            )
    ...
    async def test_regulatory_reporting(self, ms_adapter):
        ...
        regulatory_report = await ms_adapter.generate_regulatory_report(
            trade=trade, report_type="MiFID_II"
        )
