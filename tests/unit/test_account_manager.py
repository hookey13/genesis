...
@pytest.mark.asyncio
async def test_create_sub_account_with_valid_parent(account_manager, mock_repository, mock_event_bus, sample_master_account):
    ...
    mock_repository.get_account.assert_called_once_with(sample_master_account.account_id)
...
@pytest.mark.asyncio
async def test_create_sub_account_with_invalid_parent_fails(account_manager, mock_repository):
...
async def test_get_account_from_cache(account_manager, sample_master_account):
    ...
    account_manager._accounts_cache[sample_master_account.account_id] = sample_master_account
...
@pytest.mark.asyncio
async def test_get_account_from_repository(account_manager, mock_repository, sample_master_account):
    ...
    # Verify cached
    assert account_manager._accounts_cache[sample_master_account.account_id] == sample_master_account


@pytest.mark.asyncio
async def test_list_accounts_with_filters(account_manager, mock_repository, sample_master_account, sample_sub_account):
    """Test listing accounts with filters."""
    # Setup repository return
    mock_repository.list_accounts.return_value = [sample_master_account, sample_sub_account]
...
@pytest.mark.asyncio
async def test_switch_account(account_manager, mock_repository, mock_event_bus, sample_master_account):
...
@pytest.mark.asyncio
async def test_update_account_balance(account_manager, mock_repository, mock_event_bus, sample_master_account):
...
@pytest.mark.asyncio
async def test_check_account_permissions(account_manager, mock_repository, sample_master_account):
...
@pytest.mark.asyncio
async def test_update_account_permissions(account_manager, mock_repository, mock_event_bus, sample_master_account):
...
@pytest.mark.asyncio
async def test_aggregate_balances_invalid_master_fails(account_manager, mock_repository):
...
@pytest.mark.asyncio
async def test_validate_account_hierarchy_master(account_manager, mock_repository, sample_master_account):
    """Test validating master account hierarchy."""
    # Setup account
    mock_repository.get_account.return_value = sample_master_account
    
    with patch("genesis.core.account_manager.requires_tier", lambda x: lambda f: f):
        valid = await account_manager.validate_account_hierarchy(sample_master_account.account_id)
    
    assert valid  # Master account with no parent is valid


@pytest.mark.asyncio
async def test_validate_account_hierarchy_sub_with_valid_parent(account_manager, mock_repository):
...
@pytest.mark.asyncio
async def test_validate_account_hierarchy_sub_without_parent_invalid(account_manager, mock_repository):
