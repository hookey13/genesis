...
@pytest.mark.asyncio
async def test_get_risk_limits_status(
    risk_dashboard, mock_repository, mock_event_bus
):
...
async def test_handle_position_update(risk_dashboard):
    ...
    risk_dashboard._var_cache[f"{account_id}_0.95_1_historical"] = (Decimal("1000"), datetime.now())
    risk_dashboard._cvar_cache[f"{account_id}_0.95_1"] = (Decimal("1500"), datetime.now())
...
@pytest.mark.asyncio
async def test_unsupported_var_method(risk_dashboard, mock_repository, sample_positions):
