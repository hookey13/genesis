...
class TestMigration006:
    ...
    def test_migration_creates_iceberg_executions_table(self, temp_db):
        ...
        with engine.connect() as conn:
            ...
            op.create_table(
                ...
                sa.Column(
                    "slices_completed", sa.Integer, nullable=False, server_default="0"
                ),
                sa.Column(
                    "slices_failed", sa.Integer, nullable=False, server_default="0"
                ),
        ...
        # Verify columns
        columns = {
            col["name"]: col for col in inspector.get_columns("iceberg_executions")
        }
    ...
    def test_migration_creates_iceberg_slices_table(self, temp_db):
        ...
        with engine.connect() as conn:
            ...
            op.create_table(
                ...
                sa.Column(
                    "client_order_id", sa.String(64), nullable=False, unique=True
                ),
        ...
        client_order_unique = any(
            "client_order_id" in u["column_names"] for u in uniques
        )
    ...
    def test_migration_creates_market_impact_metrics_table(self, temp_db):
        ...
        with engine.connect() as conn:
            ...
            op.create_table(
                "iceberg_slices", sa.Column("slice_id", sa.String(36), primary_key=True)
            )
        ...
        columns = {
            col["name"] for col in inspector.get_columns("market_impact_metrics")
        }
        required_columns = {
            "impact_id",
            "execution_id",
    ...
    def test_migration_handles_decimal_precision(self, temp_db):
        ...
        with engine.connect() as conn:
            ...
            conn.execute(
                sa.text(
                    "INSERT INTO test_decimals (id, value_usdt, slippage_percent) VALUES (:id, :val, :slip)"
                ),
            ...
            result = conn.execute(
                sa.text("SELECT * FROM test_decimals WHERE id = 1")
            ).fetchone()
    ...
    def test_migration_rollback(self, temp_db):
        ...
        with engine.connect() as conn:
            ...
            for table in tables_to_create:
                op.create_table(table, sa.Column("id", sa.String(36), primary_key=True))
