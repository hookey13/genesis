...
class TestMigration007:
    ...
    def test_portfolio_limits_table_structure(self, alembic_config, engine):
        ...
        inspector = inspect(engine)
        columns = {
            col["name"]: col for col in inspector.get_columns("portfolio_limits")
        }
        ...
        # Check indexes
        indexes = {
            idx["name"]: idx for idx in inspector.get_indexes("portfolio_limits")
        }
    ...
    def test_pair_performance_table_structure(self, alembic_config, engine):
        ...
        inspector = inspect(engine)
        columns = {
            col["name"]: col for col in inspector.get_columns("pair_performance")
        }
        ...
        # Check indexes
        indexes = {
            idx["name"]: idx for idx in inspector.get_indexes("pair_performance")
        }
    ...
    def test_position_correlations_enhanced(self, alembic_config, engine, session):
        ...
        inspector = inspect(engine)
        columns = {
            col["name"]: col for col in inspector.get_columns("position_correlations")
        }
        ...
        # Check new index
        indexes = {
            idx["name"]: idx for idx in inspector.get_indexes("position_correlations")
        }
    ...
    def test_migration_down_removes_tables(self, alembic_config, engine):
        ...
        columns = {
            col["name"] for col in inspector.get_columns("position_correlations")
        }
