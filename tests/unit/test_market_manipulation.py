...
class TestMarketManipulationDetector:
    ...
    async def test_track_order_cancellation(self, detector):
        ...
        # Cancel order
        await detector.track_order_cancellation(symbol="BTCUSDT", order_id="order_1")
    ...
    def test_calculate_levels_from_best(self, detector):
        ...
        # Test bid side
        assert (
            detector._calculate_levels_from_best("BTCUSDT", Decimal("50000"), "bid")
            == 0
        )
        assert (
            detector._calculate_levels_from_best("BTCUSDT", Decimal("49999"), "bid")
            == 1
        )
        assert (
            detector._calculate_levels_from_best("BTCUSDT", Decimal("49997"), "bid")
            == 3
        )

        # Test ask side
        assert (
            detector._calculate_levels_from_best("BTCUSDT", Decimal("50001"), "ask")
            == 0
        )
        assert (
            detector._calculate_levels_from_best("BTCUSDT", Decimal("50002"), "ask")
            == 1
        )
    ...
    def test_severity_determination(self, detector):
        ...
        # High severity
        assert (
            detector._determine_severity(Decimal("0.95"), Decimal("2000000")) == "high"
        )

        # Medium severity
        assert (
            detector._determine_severity(Decimal("0.75"), Decimal("200000")) == "medium"
        )
    ...
    async def test_quote_stuffing_detection(self, detector, event_bus):
        ...
        for i in range(60):  # 60 orders in quick succession
            ...
            detector.order_history["BTCUSDT"].append(
                OrderActivity(
                    f"stuff_{i}",
                    "BTCUSDT",
