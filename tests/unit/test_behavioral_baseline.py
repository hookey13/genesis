...
class TestBehavioralBaseline:
        ...
        for day in range(35):
            ...
            for hour in [9, 12, 15, 18]:
                metrics.append(BehavioralMetric(
                    ...
                    value=Decimal(str(100 + (hour * 5) + (day % 7) * 10)),  # 100-200ms range
    ...
    def test_calculate_baseline_with_empty_metrics(self, baseline_calculator):
        ...
        with pytest.raises(ValueError, match="Cannot calculate baseline from empty metrics"):
            baseline_calculator.calculate_baseline([])
    
    def test_calculate_baseline_with_sufficient_data(self, baseline_calculator, sample_metrics):
    ...
    def test_metric_range_calculation(self, baseline_calculator):
        ...
        values = [Decimal(str(x)) for x in [10, 15, 20, 25, 30, 35, 40, 100]]  # 100 is outlier
...
class TestBehavioralMetricCollector:
    ...
    def test_collect_order_placed_metric(self, collector):
        ...
        collector.last_action_time = datetime.now(timezone.utc) - timedelta(milliseconds=250)
...
class TestBaselineProfile:
    ...
    def test_baseline_profile_creation(self):
        ...
        profile = BaselineProfile(
            profile_id="test_profile",
            learning_start_date=datetime.now(timezone.utc),
            is_mature=False,
            context="normal"
        )
