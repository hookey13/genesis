...
class TestSpreadAnalyzer:
    ...
    def test_track_spread_valid(self, analyzer):
        ...
        assert abs(metrics.spread_bps - Decimal("10")) < Decimal(
            "0.01"
        )  # 0.1% = 10 bps
    ...
    def test_calculate_spread_bps(self, analyzer):
        ...
        assert abs(spread - Decimal("100")) < Decimal(
            "0.5"
        )  # Allow for rounding in wider spreads
        ...
        # Invalid inputs
        assert analyzer.calculate_spread_bps(Decimal("0"), Decimal("100")) == Decimal(
            "0"
        )
        assert analyzer.calculate_spread_bps(Decimal("100"), Decimal("0")) == Decimal(
            "0"
        )
    ...
    def test_order_imbalance_dataclass(self):
        ...
        imbalance = OrderImbalance(
            ratio=Decimal("1.2"), bid_weight=Decimal("120"), ask_weight=Decimal("100")
        )
        assert not imbalance.is_significant

        # Significant bid pressure
        imbalance_bid = OrderImbalance(
            ratio=Decimal("2.5"), bid_weight=Decimal("250"), ask_weight=Decimal("100")
        )
        assert imbalance_bid.is_significant

        # Significant ask pressure
        imbalance_ask = OrderImbalance(
            ratio=Decimal("0.4"), bid_weight=Decimal("40"), ask_weight=Decimal("100")
        )
